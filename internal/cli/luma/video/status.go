...
func runStatus(cmd *cobra.Command, args []string, flags *statusFlags) error {
	...
	// Include asset URLs in verbose mode or when completed
	if flags.verbose && gen.Assets != nil {
		assets := make(map[string]string)
		if gen.Assets.Video != "" {
			assets["video"] = gen.Assets.Video
		}
		if gen.Assets.Image != "" {
			assets["image"] = gen.Assets.Image
		}
		if gen.Assets.ProgressVideo != "" {
			assets["progress_video"] = gen.Assets.ProgressVideo
		}
		if len(assets) > 0 {
