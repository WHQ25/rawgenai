...
func newDownloadCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:         "Download completed audio",
		Long:          "Download the result of a completed audio generation task.",
		SilenceErrors: true,
		SilenceUsage:  true,
...
func runDownload(cmd *cobra.Command, args []string, flags *downloadFlags) error {
	...
	if ext != ".mp3" && ext != ".wav" && ext != ".mp4" {
		return common.WriteError(cmd, "invalid_output", "output file must have .mp3, .wav, or .mp4 extension")
	}
